...
function normalizeSearchResult(item: Record<string, any> = {}) {
  return {
...
async function callTavily(payload: Record<string, any>, useBodyApiKey = false) {
  ...
  if (!response.ok) {
    const error: any = new Error(data?.error || data?.message || `Tavily HTTP ${response.status}`);
...
export async function searchWeb(query: string, options: { searchDepth?: string; maxResults?: number } = {}) {
  ...
  try {
    ...
    return {
      ok: true,
      answer: String(data.answer || "").trim().slice(0, 1000),
      results: Array.isArray(data.results)
        ? data.results.map(normalizeSearchResult).filter((item: any) => item.title || item.content).slice(0, 5)
        : [],
    };
  } catch (error: any) {
...
export function createTavilySearchSkill(_ctx: FastFoodContext) {
  return createTool({
    name: "searchWeb",
