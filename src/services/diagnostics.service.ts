...
export async function checkAlemiHub(
  ...
  hydrate: (instanceId: string) => Promise<Record<string, any> | null> = getRestaurantConfig
): Promise<CheckResult[]> {
  const indexed = await loadConfigs().catch(() => [] as Record<string, any>[]);
  const instances = [...new Set(
    indexed.map((config) => String(config?.instance_id || config?.instance || "").trim()).filter(Boolean)
  )];
  if (!instances.length) {
