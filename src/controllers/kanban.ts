...
export async function handleKanbanWebhook(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    let textMessage = "";
    if (action === "new_order") textMessage = buildNewOrderMessage(body, lang, orderId, isPickup);
    if (action === "request_payment") textMessage = await buildPaymentMessage(body, config, lang, instance);
    if (action === "order_rejected") textMessage = buildRejectedMessage(body, lang);
