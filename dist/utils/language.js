...
export function parseGeminiLanguageDecision(value) {
    ...
    try {
        ...
        if (language)
            parsed = { language, confidence: 0.7 };
    }
...
export async function detectLanguageDecision(text, classifier = generateMediaText) {
    if (!isLanguageBearingCustomerText(text))
    ...
    try {
        ...
        if (!parsed)
            throw new Error("INVALID_GEMINI_LANGUAGE_JSON");
