...
const ACTIVE_ORDER_FOLLOW_UP_RE = /((?:че|чё|что)\s*там|ну\s*и|и\s*что|не\s*болды|не\s*жаңалық|қалай\s*болып\s*жатыр|как\s*там|дайын\s*ба|дайынба|готов(?:о|а)?\s*ли|готов(?:о|а)?|қашан|когда|скоро\s*ма|скоро|долго\s*(?:ещ[её])?|сколько\s*(?:ещ[её])?|әлі\s*(?:көп\s*пе|қанша\s*күт)|қанша\s*күт|жолда\s*ма|курьер|едет|келе\s*жатыр\s*ма)/iu;
...
const PROSPECTIVE_ORDER_TIMING_RE = /((?:қазір|казир|жаңа|жана|сейчас|новый|если)\s+(?:заказ|тапсырыс).{0,32}(?:берсем|жасасам|берем|жасайм|закажу|оформлю|сделаю|дам)|(?:заказ|тапсырыс).{0,24}(?:берсем|жасасам|берем|закажу|оформлю|сделаю))/iu;
...
export function isUnownedOrderTimingQuestion(options) {
    if (options.hasActiveOrder)
        return false;
    if (options.quotedOrderNumber || options.discussedOrderNumber)
        return false;
...
export function isLikelyOrderStatusFollowUp(text = "") {
    const value = String(text || "");
    if (!intentMatches(ACTIVE_ORDER_FOLLOW_UP_RE, value))
        return false;
    // An explicit order reference keeps the status route even while browsing.
    if (requestedOrderNumber(value) || intentMatches(ORDER_STATUS_QUESTION_RE, value))
        return true;
