...
export function buildMagicLink(domain, phone) {
    const secret = process.env.CRM_SECRET_TOKEN;
    if (!domain || !phone || !secret)
        return null;
...
export function hasExplicitMenuLinkIntent(text) {
    ...
    return (/(сілтеме|ссылка|link|линк|мәзір|меню|каталог|тапсырыс\s*бер|заказ\s*бер|заказать|оформить|корзин|себет|меню жібер|мәзір жібер|меню бер|мәзір бер|қалай заказ|қалай тапсырыс)/iu.test(value) ||
