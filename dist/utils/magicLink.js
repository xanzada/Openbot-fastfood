...
function normalizeMenuDomain(domain) {
...
function generateSecureMenuUrl(domain, phone, tenantSecret = "") {
  ...
  const hash = crypto.createHash("sha256").update(`${cleanPhone}${secret}${timestamp}`, "utf8").digest("hex");
...
function isMenuLinkResendRequest(text = "") {
...
function hasExplicitMenuLinkIntent(text) {
  const value = String(text || "").toLowerCase();
  return /(сілтеме|ссылка|link|линк|мәзір|меню|каталог|тапсырыс\s*(бер|берейін|берем|жасай|ет|қыл)|заказ\s*(бер|берейін|берем|жасай|ет|қыл|хочу|сдел|оформ)|заказать|оформить|хочу\s+заказ|хочу\s+заказать|корзин|себет|меню жібер|мәзір жібер|меню бер|мәзір бер|қалай заказ|қалай тапсырыс)/iu.test(value) || isMenuLinkResendRequest(value);
}
export {
  generateSecureMenuUrl,
