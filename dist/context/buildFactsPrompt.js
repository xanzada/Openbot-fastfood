...
function compactConversationHistory(history) {
  const normalized = (Array.isArray(history) ? history : []).map((entry, index) => ({
    ...
    index
  })).filter((entry) => Boolean(entry.role && entry.text)).sort((a, b) => a.createdAt - b.createdAt || a.index - b.index);
  let customerCount = 0;
...
function operationalRuntime(ctx) {
  ...
  return {
    wait_time: Number(live.wait_time || 0),
    delivery: live.delivery ?? null,
    pickup: live.pickup ?? null,
    is_emergency: Boolean(live.is_emergency),
    reset_at: Number(live.reset_at || 0),
    stale: Boolean(live.stale),
    runtime_available: Boolean(live.runtime_available)
  };
}
