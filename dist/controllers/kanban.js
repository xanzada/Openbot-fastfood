...
function legacyPaymentDetailsFromPayloadRuntimeOrConfig(body, runtimeStatus, config) {
    const payloadDetails = normalizePaymentDetails(body.payment_details || body.paymentDetails || body.requisites);
...
function legacyPaymentDetailsSource(body, runtimeStatus, config) {
    if (normalizePaymentDetails(body.payment_details || body.paymentDetails || body.requisites).length)
        return "payload";
    if (paymentDetailsFromRuntime(runtimeStatus).length)
        return "runtime";
    if (paymentDetailsFromConfig(config).length)
