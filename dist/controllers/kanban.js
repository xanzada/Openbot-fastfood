...
import { connectRedis, clearKitchenCheckoutState, deleteShiftNote, getKitchenStatus, getSiteLanguageHint, getUserLang, redisClient, saveKitchenStatus, saveSiteLanguageHint, saveShiftNote, saveToHistory, } from "../services/redis.service.js";
...
export function boolValue(value, fallback = false) {
    ...
    if (PICKUP_WORDS.includes(normalized) || PICKUP_WORDS.includes(collapsed))
        return true;
    if (DELIVERY_WORDS.includes(normalized) || DELIVERY_WORDS.includes(collapsed))
        return false;
    return fallback;
}
...
export function resolveStatusTemplateKey(status, isPickup) {
    ...
    if (normalized === "ready")
        return isPickup ? "pickup_ready" : "ready_delivery";
    if (normalized === "completed" && isPickup)
        return "pickup_ready";
    return normalized;
}
export function extractShiftNotePayload(body) {
...
function printJobIdOf(body) {
    const order = body.order && typeof body.order === "object" ? body.order : {};
    return textValue(body.print_job_id || body.printJobId || order.print_job_id || order.printJobId);
}
async function reportPrintDispatch(io, body, scope) {
 ...
 */
export async function claimInboundEvent(instance, rawEventId) {
    const eventId = cleanInline(String(rawEventId ?? ""), 80);
    if (!eventId)
        return { eventId: "", key: "", claimed: true };
    ...
    try {
        ...
        if (!VALID_ACTIONS.has(action)) {
            ...
            auditError("Rejected webhook: invalid action", new Error("BAD_ACTION"), {
                ...
                bodyShape: req.inboundBodyShape || describeBodyShape(body),
            });
