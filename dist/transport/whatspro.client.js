...
function splitWhatsProResponse(text = "") {
...
async function sendWhatsProMessage(payload) {
...
async function sendWhatsProPresence(payload) {
  const transport = await resolveWhatsProTransport(payload.instanceId);
  ...
  try {
    const response = await axios.post(
      ...
      { timeout: 3e3, headers: whatsproHeaders(transport.apiToken) }
    );
...
function startWhatsProTyping(payload) {
  let stopped = false;
  const pulse = () => {
    if (!stopped) void sendWhatsProPresence(payload).catch(() => void 0);
  };
  pulse();
  const timer = setInterval(pulse, 4e3);
  timer.unref?.();
  return () => {
    stopped = true;
    clearInterval(timer);
...
async function sendWhatsProResponseSequence(payload) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    if (!result?.acknowledged) {
      await connectRedis().then(() => redisClient.setEx(`outbox:whatspro:${outboundId}`, 24 * 60 * 60, JSON.stringify({ id: outboundId, instanceId: payload.instanceId, phone: payload.phone, text: chunks[index], attempts: 3, failedAt: Date.now(), error: String(lastError?.message || "delivery_not_acknowledged") }))).catch(() => void 0);
      throw lastError || new Error("WHATSPRO_DELIVERY_NOT_ACKNOWLEDGED");
    }
    await connectRedis().then(() => redisClient.del(`outbox:whatspro:${outboundId}`)).catch(() => void 0);
    sent.push(result);
  }
...
export {
  sendWhatsProMessage,
  sendWhatsProPresence,
  sendWhatsProResponseSequence,
  splitWhatsProResponse,
  startWhatsProTyping
};
