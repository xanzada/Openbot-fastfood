...
function selectPublicMenuItems(items, query = "", category = "", limit = 12) {
  const normalizedQuery = normalizeText(query);
  const normalizedCategory = normalizeText(category);
  const tokens = normalizedQuery.split(/\s+/).filter(Boolean);
  const max = Math.min(50, Math.max(1, Number(limit) || 12));
  return items.map((item) => ({ item, score: scoreMenuItem(item, tokens, normalizedQuery) })).filter((entry) => {
  ...
  }).sort((left, right) => right.score - left.score || Number(left.item.price || 0) - Number(right.item.price || 0)).slice(0, max).map((entry) => ({
    name: entry.item.name,
    category: entry.item.category_name,
    ingredients: entry.item.composition || entry.item.description || "",
    price: entry.item.price,
    available: typeof entry.item.available === "boolean" ? entry.item.available : void 0
  }));
}
function createSearchMenuSkill(ctx) {
  return createTool({
    name: "searchMenu",
...
export {
  createSearchMenuSkill,
  selectPublicMenuItems
};
