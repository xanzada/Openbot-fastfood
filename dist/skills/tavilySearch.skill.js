...
export async function searchWeb(query, options = {}) {
    ...
    if (!cleanQuery)
        return { ok: false, error: "EMPTY_QUERY", answer: "", results: [] };
    if (!isTavilyConfigured())
        return { ok: false, error: "TAVILY_NOT_CONFIGURED", answer: "", results: [] };
    const payload = {
        query: cleanQuery,
    ...
    try {
        ...
        return {
            ...
            results: Array.isArray(data.results)
                ? data.results.map(normalizeSearchResult).filter((item) => item.title || item.content).slice(0, 5)
                : [],
        };
    }
    catch (error) {
...
export function createTavilySearchSkill(_ctx) {
    return createTool({
        name: "searchWeb",
