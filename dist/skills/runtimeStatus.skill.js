...
export function createGetKitchenStatusSkill(ctx) {
    return createTool({
        ...
        execute: async () => {
            ...
            const fromFallback = Boolean(status?.redis_runtime_fallback || status?.stale_runtime_backup || /fallback|stale|backup/.test(source));
            return {
                source,
...
export function createGetShiftNotesSkill(ctx) {
    return createTool({
        name: "getShiftNotes",
