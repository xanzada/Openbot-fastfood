...
function compactStringArray(value, maxItems = 8, maxLength = 120) {
  ...
  return source.map((item) => cleanPromptLine(item, maxLength)).filter(Boolean).filter((item, index, arr) => arr.findIndex((other) => other.toLowerCase() === item.toLowerCase()) === index).slice(0, maxItems);
}
...
async function getRestaurantConfig(instanceId) {
...
async function getAllRestaurantConfigs() {
  ...
  try {
    ...
    const records = Array.isArray(response.data?.list) ? response.data.list.map((record) => normalizeRestaurantConfig(record)).filter((record) => String(record?.instance_id || record?.instance || "").trim()).filter(Boolean) : [];
...
async function getRestaurantConfigByWhatsAppPhone(phone) {
...
function selectRelevantShpor(records = [], query = "", limit = SHPOR_CONTEXT_LIMIT) {
  ...
  if (safeQuery === "*") return records.slice(0, 100);
  if (!safeQuery) return [];
  return records.map((item) => ({ item, score: scoreShporRecord(item, safeQuery) })).filter((row) => row.score > 0).sort((a, b) => b.score - a.score).slice(0, Math.max(1, limit)).map((row) => row.item);
}
async function getShporContext(instanceId, query = "") {
...
function buildShporSavePayload(instanceId, question, answer, category = "general", memoryPayload = null) {
  ...
  const keywords = Array.isArray(memory.keywords) ? memory.keywords.map((item) => cleanInline(item, 42)).filter(Boolean).slice(0, 10) : [];
  const compact = {
    ...
    saved_at: memory.saved_at || (/* @__PURE__ */ new Date()).toISOString()
  };
  return {
    instance_id: instanceId,
...
async function saveToShpor(instanceId, question, answer, category = "general", memoryPayload = null) {
  try {
    const tableId = process.env.NOCODB_SHPOR_TABLE_ID || "";
...
async function evaluateForShpor(question, answer) {
  try {
      ...
      tools: {
        record_shpor_evaluation: tool({
          ...
          inputSchema: z.object({
            ...
            tool_policy: z.string().optional().describe("Recommended action: reply_to_customer, search_menu, escalate_to_admin, escalate_to_human, cancel_order, or register_payment_receipt."),
...
export {
  ...
  getRestaurantConfig,
  getRestaurantConfigByWhatsAppPhone,
  getShporContext,
  saveToShpor
};
