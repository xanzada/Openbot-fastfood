...
function receiptFilterEnabled(env = process.env) {
...
function missingSender(value) {
  ...
  return !sender || /^(белгісіз|неизвестно|unknown|sender|отправитель)$/iu.test(sender) || /[^\p{L}\s.'’\-]/u.test(sender) || nameParts.length < 2;
}
function missingBank(value) {
  const bank = String(value || "").trim();
  return !bank || /(белгісіз|неизвест|unknown|анықталма|not[\s_-]*found)/iu.test(bank) || !/[\p{L}]{3,}/u.test(bank);
}
function validateReceiptAnalysis(analysis, context = {}) {
  ...
  if (receiptTime > now + 10 * 6e4) return { valid: false, reason: "receipt_in_future" };
  if (now - receiptTime > 24 * 60 * 6e4) return { valid: false, reason: "receipt_too_old" };
  const orderTime = Date.parse(String(context.orderCreatedAt || ""));
  if (Number.isFinite(orderTime) && receiptTime < orderTime - 15 * 6e4) return { valid: false, reason: "receipt_before_order" };
  return { valid: true, reason: "ok" };
}
function createReceiptFingerprint(base64Media, analysis) {
  return crypto.createHash("sha256").update(stripDataUrl(base64Media)).update(`|${analysis.amount}|${analysis.bank_name}|${analysis.sender_name}|${analysis.date_time}|${analysis.transaction_id || ""}`).digest("hex");
}
...
function fallbackTechnicalError(error, userLang) {
  ...
  const reply = userLang === "ru" ? "\u0418\u0437\u0432\u0438\u043D\u0438\u0442\u0435, \u0441\u0435\u0439\u0447\u0430\u0441 \u043D\u0435 \u043F\u043E\u043B\u0443\u0447\u0438\u043B\u043E\u0441\u044C \u043E\u0431\u0440\u0430\u0431\u043E\u0442\u0430\u0442\u044C \u0444\u0430\u0439\u043B. \u041F\u043E\u043F\u0440\u043E\u0431\u0443\u0439\u0442\u0435 \u043E\u0442\u043F\u0440\u0430\u0432\u0438\u0442\u044C \u0435\u0433\u043E \u0435\u0449\u0451 \u0440\u0430\u0437 \u0447\u0443\u0442\u044C \u043F\u043E\u0437\u0436\u0435." : "\u041A\u0435\u0448\u0456\u0440\u0456\u04A3\u0456\u0437, \u0444\u0430\u0439\u043B\u0434\u044B \u049B\u0430\u0437\u0456\u0440 \u04E9\u04A3\u0434\u0435\u0439 \u0430\u043B\u043C\u0430\u0434\u044B\u043C. \u0421\u04D9\u043B\u0434\u0435\u043D \u0441\u043E\u04A3 \u049B\u0430\u0439\u0442\u0430 \u0436\u0456\u0431\u0435\u0440\u0456\u043F \u043A\u04E9\u0440\u0456\u04A3\u0456\u0437.";
  return {
...
function buildMediaPrompt(mimeType, caption, userLang, isPdf, receiptContext) {
  const pdfInstruction = isPdf ? "This is a PDF document. It is usually a bank receipt or payment confirmation. Carefully extract the amount, bank name, and date." : mimeType.startsWith("audio/") ? "This is an audio/voice message. Transcribe the customer's intent and identify receipts, payment confirmations, complaints, or admin escalation needs." : "This is an image. If it shows a bank transfer, Kaspi/Halyk/Jusan screenshot, or a receipt, treat it as a receipt. If it shows food defects, hair, dirt, or a wrong order, treat it as a complaint.";
  return `
[MEDIA TOOL TASK]
...
async function analyzeMedia(base64Media, mimeType, caption = "", userLang = "kk", isPdf = false, systemPrompt = "", receiptContext = {}) {
  try {
