...
const localProcessing = new Map();
const localDuplicateText = new Map();
const localSpam = new Map();
const localMediaQuota = new Map();
const localInboundBuffers = new Map();
function localKey(instanceId, value) {
    return `${instanceId}:${value}`;
}
function pruneLocalGuardState(now = Date.now()) {
    for (const [key, expiresAt] of localDone)
        if (expiresAt <= now)
            localDone.delete(key);
    for (const [key, expiresAt] of localProcessing)
        if (expiresAt <= now)
            localProcessing.delete(key);
    for (const [key, item] of localDuplicateText)
        if (item.expiresAt <= now)
            localDuplicateText.delete(key);
    for (const [key, item] of localSpam) {
        if (item.windowEndsAt <= now && item.mutedUntil <= now)
            localSpam.delete(key);
    }
    for (const [key, item] of localMediaQuota)
        if (item.windowEndsAt <= now)
            localMediaQuota.delete(key);
    for (const [key, item] of localInboundBuffers)
        if (item.expiresAt <= now)
            localInboundBuffers.delete(key);
}
function guardIncomingMessageInMemory(instanceId, phone, text, messageId) {
    const now = Date.now();
    pruneLocalGuardState(now);
    const messageKey = messageId ? localKey(instanceId, messageId) : "";
    if (messageKey && localDone.has(messageKey))
        return { blocked: true, reason: "duplicate_done_local" };
    if (messageKey && localProcessing.has(messageKey))
        return { blocked: true, reason: "duplicate_processing_local" };
    if (messageKey)
        localProcessing.set(messageKey, now + PROCESSING_LOCK_SECONDS * 1000);
    const customerKey = localKey(instanceId, phone);
    const spam = localSpam.get(customerKey);
    if (spam?.mutedUntil && spam.mutedUntil > now)
        return { blocked: true, reason: "spam_muted_local" };
    if (text) {
    ...
    activeSpam.count += 1;
    if (activeSpam.count > SPAM_LIMIT)
        activeSpam.mutedUntil = now + MUTE_SECONDS * 1000;
...
function mediaSignatureMatches(mimeType, base64) {
    try {
        ...
        if (["audio/ogg", "audio/opus"].includes(mime))
        ...
        if (["audio/mpeg", "audio/mp3"].includes(mime))
            return ascii.startsWith("ID3") || (b[0] === 0xff && (b[1] & 0xe0) === 0xe0);
        if (["audio/webm"].includes(mime))
            return hex.startsWith("1a45dfa3");
        if (["audio/mp4", "audio/m4a"].includes(mime))
...
export function extractInboundMedia(body) {
    ...
    if (kind !== "sticker" && kind !== "video" && sizeBytes > maxBytesForKind(kind))
        flags.push("media_too_large");
    if (kind === "video")
        flags.push("video_unsupported");
    if (kind === "audio" && !isVoiceNote)
        flags.push("music_audio_not_supported");
    if (kind === "audio" && durationSeconds > MAX_VOICE_SECONDS)
        flags.push("voice_too_long");
    if (kind === "audio" &&
        ![
            "audio/ogg",
            "audio/opus",
...
export function safeMediaMetadata(mediaContext) {
    if (!mediaContext)
        return null;
    return {
        hasMedia: true,
    ...
    try {
        ...
        while (offset + 27 <= buffer.length && buffer.subarray(offset, offset + 4).toString("ascii") === "OggS") {
            ...
            for (let index = 0; index < segmentCount; index += 1)
                bodyLength += buffer[offset + 27 + index];
            const pageLength = 27 + segmentCount + bodyLength;
            if (offset + pageLength > buffer.length)
                break;
            const granule = buffer.readBigUInt64LE(offset + 6);
            if (granule !== 0xffffffffffffffffn && granule > maxGranule)
                maxGranule = granule;
            offset += pageLength;
        }
...
function directMediaBase64(body) {
    return firstString(body?.base64, body?.dataUrl, body?.mediaData, body?.media?.base64, body?.media?.data, body?.media?.dataUrl, body?.data?.base64, body?.data?.mediaData, body?.data?.media?.base64, body?.message?.base64);
}
...
export async function getBase64Media(body, mediaContext = extractInboundMedia(body)) {
    ...
    try {
        ...
        return { error: error?.message === "MEDIA_TOO_LARGE" ? "media_too_large" : "media_download_failed" };
    }
}
...
function isMimeRecoverable(mediaContext) {
...
export async function hydrateInboundMedia(body, mediaContext) {
    if (!mediaContext)
        return null;
    if (!mediaContext.valid && !isMimeRecoverable(mediaContext))
        return mediaContext;
    if (mediaContext.kind === "sticker" || mediaContext.kind === "video")
        return mediaContext;
    const downloaded = await getBase64Media(body, mediaContext);
    if (!downloaded)
        return { ...mediaContext, valid: false, reason: "media_download_failed", flags: [...mediaContext.flags, "media_download_failed"] };
    if ("error" in downloaded)
        return { ...mediaContext, valid: false, reason: downloaded.error, flags: Array.from(new Set([...mediaContext.flags, downloaded.error])) };
    if (!downloaded.base64)
        return { ...mediaContext, valid: false, reason: "media_download_failed", flags: [...mediaContext.flags, "media_download_failed"] };
    const downloadedMime = normalizeMimeBase(downloaded.mimeType || mediaContext.mimeType);
    if (!ALLOWED_MEDIA_MIME.test(downloadedMime))
        return { ...mediaContext, valid: false, reason: "unsupported_mime_type", flags: [...mediaContext.flags, "unsupported_mime_type"] };
    if (!mediaSignatureMatches(downloadedMime, downloaded.base64))
        return { ...mediaContext, valid: false, reason: "media_signature_mismatch", flags: Array.from(new Set([...mediaContext.flags, "media_signature_mismatch"])) };
    if (mediaContext.kind === "audio" && mediaContext.isVoiceNote && !mediaContext.durationSeconds && !["audio/ogg", "audio/opus"].includes(downloadedMime))
        return { ...mediaContext, valid: false, reason: "voice_duration_unverified", flags: Array.from(new Set([...mediaContext.flags, "voice_duration_unverified"])) };
...
export function testModeAllowedPhones(config, env = process.env) {
    return new Set([
...
async function getTestModeAllowedPhones(instanceId) {
...
export async function saveMediaContext(instanceId, phone, mediaContext) {
    try {
        await connectRedis();
        await redisClient.setEx(`media_context:${instanceId}:${phone}`, MEDIA_CONTEXT_SECONDS, JSON.stringify({ ...safeMediaMetadata(mediaContext), savedAt: Date.now() }));
    }
    catch {
        // Media processing can continue from the current request context.
    }
}
export async function bufferInboundText(input) {
    const instanceId = String(input.instanceId || "").trim();
    const phone = String(input.phone || "").replace(/\D/g, "");
    ...
    if (!instanceId || !phone || !text)
        return { leader: true, text, parts: text ? 1 : 0, items: text ? [text] : [] };
    try {
        await connectRedis();
        ...
        if ((await redisClient.get(latestKey)) !== token)
            return { leader: false, text: "", parts: 0, items: [] };
        const rows = await redisClient.lRange(listKey, 0, -1);
        await redisClient.del([listKey, latestKey]);
        const parts = rows.map((row) => { try {
        ...
        if (!latest || latest.latestToken !== token)
            return { leader: false, text: "", parts: 0, items: [] };
        localInboundBuffers.delete(key);
...
export async function claimMediaAiQuota(instanceId, phone) {
    try {
        await connectRedis();
...
const localTurnLocks = new Map();
export async function acquireTurnLock(instanceId, phone, ttlMs = TURN_LOCK_FALLBACK_TTL_MS) {
...
export async function releaseTurnLock(instanceId, phone, owner) {
    const key = `turn_lock:${instanceId}:${phone}`;
    try {
        await connectRedis();
 ...
 */
export async function drainInboundBuffer(instanceId, phone) {
    const listKey = `inbound_buffer:${instanceId}:${phone}`;
    const latestKey = `inbound_buffer_latest:${instanceId}:${phone}`;
    try {
        await connectRedis();
