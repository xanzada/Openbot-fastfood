...
async function callGeminiChannel(request, keys, model, label) {
    const transientErrors = [];
    const hardErrors = [];
    const start = keys.length ? mediaKeyCursor % keys.length : 0;
    if (keys.length)
        mediaKeyCursor = (mediaKeyCursor + 1) % keys.length;
    for (let attempt = 0; attempt < keys.length; attempt += 1) {
        ...
        try {
            ...
            if (!text)
                throw new Error("GEMINI_MEDIA_EMPTY_RESPONSE");
...
export async function callOpenRouter(request) {
    ...
    if (!text)
        throw new Error("OPENROUTER_MEDIA_EMPTY_RESPONSE");
...
export async function generateMediaText(request) {
