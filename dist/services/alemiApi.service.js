...
function tenantEnvironmentEntry(instanceId, env) {
...
export function resolveAlemiCredentials(instanceId, config, env = process.env) {
    ...
    if (!secret)
        throw new Error("ALEMI_SECRET_NOT_CONFIGURED");
    return { apiUrl, instance, secret };
}
function unixTimestamp(nowMs = Date.now()) {
    if (!Number.isFinite(nowMs) || nowMs <= 0)
        throw new Error("ALEMI_INVALID_TIMESTAMP");
...
function signature(secret, timestamp, signedBytes) {
    return `v1=${crypto.createHmac("sha256", secret).update(`${timestamp}.${signedBytes}`, "utf8").digest("hex")}`;
}
function commonHeaders(credentials, commandId, timestamp, signedBytes) {
    return {
...
const axiosTransport = async (request) => {
...
export function unwrapAlemiResponse(value) {
    let current = value;
...
function assertAlemiResponse(response) {
    if (response.status < 200 || response.status >= 300) {
        const error = new Error(`ALEMI_HTTP_${response.status}`);
    ...
    if (raw?.ok === false || raw?.success === false || unwrapped?.ok === false || unwrapped?.success === false) {
        const error = new Error("ALEMI_COMMAND_REJECTED");
        error.statusCode = response.status;
...
async function withRotatedSecretRetry(instanceId, options, send) {
...
export async function callAlemiCommand(instanceId, command, data, options = {}) {
...
export async function callAlemiLegacyAction(action, payload, options = {}) {
...
export async function issueCustomerAccessLink(input, options = {}) {
    const phone = e164Kazakhstan(input.phone);
    if (!phone)
        return null;
    const result = await callAlemiCommand(input.instanceId, "customer.access_link.issue", { phone_e164: phone, locale: input.locale }, { ...options, config: options.config === undefined ? input.config : options.config });
    if (typeof result === "string")
        return httpUrlOrNull(result.trim());
...
export async function uploadOrderDocument(input, options = {}) {
    ...
    return withRotatedSecretRetry(input.instanceId, options, async (config) => {
        ...
        if (!orderId)
            throw new Error("ALEMI_ORDER_ID_REQUIRED");
        if (!sourceMessageId)
            throw new Error("ALEMI_SOURCE_MESSAGE_ID_REQUIRED");
        if (!mimeType || !input.bytes?.byteLength)
        ...
        form.append("file", new Blob([blobBytes], { type: mimeType }), input.fileName || `receipt.${extensionForMime(mimeType)}`);
        const response = await (options.transport || axiosTransport)({
...
export async function reportPrintResult(input, options = {}) {
    ...
    return withRotatedSecretRetry(input.instanceId, options, async (config) => {
        ...
        if (!printJobId)
            throw new Error("ALEMI_PRINT_JOB_ID_REQUIRED");
        if (status !== "completed" && status !== "failed")
            throw new Error("ALEMI_PRINT_STATUS_INVALID");
        const canonical = [
            "print-result-v1",
            commandId,
            credentials.instance,
