...
export function describeOrderStatus(status, language) {
    const key = statusKey(status);
    const isDelivery = /(^|_)(delivery|out_of_delivery|out_for_delivery|on_the_way|courier)(_|$)/.test(key);
    if (isDelivery)
        return language === "ru" ? "курьер в пути" : "жеткізу жолында";
    if (/(prepar|cook|готовит|дайында|әзірле)/u.test(key))
        return language === "ru" ? "готовится" : "дайындалып жатыр";
    if (/(ready|готов|дайын)/u.test(key))
        return language === "ru" ? "готов к выдаче" : "алып кетуге дайын";
    if (/(paid|confirm|accepted|принят|қабылдан)/u.test(key))
        return language === "ru" ? "принят и ожидает приготовления" : "қабылданды және дайындалуын күтуде";
    if (/(cancel|reject|отмен)/u.test(key))
        return language === "ru" ? "отменён" : "бас тартылды";
    if (/(complete|done|finish|закрыт|аяқтал)/u.test(key))
        return language === "ru" ? "завершён" : "аяқталды";
...
export function customerOrderFromContext(context, expectedPhone, language, hasRequestedOrderNumber = false) {
    if (!context)
        return { state: "not_found" };
    if (context.is_stale)
...
export async function getCustomerOrder(instanceId, domain, phone, language, orderNumber) {
    try {
...
export function formatCustomerOrderStatus(order, language) {
