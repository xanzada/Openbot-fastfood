...
function scalarSecret(value) {
    return typeof value === "string" ? value.trim() : "";
}
export function getIncomingTenantSecret(req) {
    const candidates = [
...
function getTenantSecrets(config, channel = "webhook") {
    if (!config)
        return [];
...
function alemiEnvironmentSecret(instanceId) {
...
export function assertTenantSecret(req, config, channel = "webhook") {
    ...
    if (channel === "kanban") {
        ...
        if (environmentSecret && !expected.includes(environmentSecret))
            expected.push(environmentSecret);
        const deploymentSecret = alemiDeploymentSecret(req, config);
        if (deploymentSecret && !expected.includes(deploymentSecret))
            expected.push(deploymentSecret);
    }
