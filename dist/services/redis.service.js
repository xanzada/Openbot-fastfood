...
export async function savePendingKitchenConsent(instanceId, phone, policyFingerprint, kind = "delay") {
    return safeRedis(false, async () => {
        const result = await redisClient.set(kitchenConsentKey(instanceId, phone), JSON.stringify({ policyFingerprint, kind, createdAt: Date.now() }), { EX: KITCHEN_CONSENT_TTL_SECONDS });
        return result === "OK";
    });
}
export async function getPendingKitchenConsent(instanceId, phone) {
...
export async function clearPendingKitchenConsent(instanceId, phone) {
...
export async function markKitchenCheckoutStarted(instanceId, phone, policyFingerprint = "") {
    return safeRedis(false, async () => {
...
export async function hasActiveKitchenCheckout(instanceId, phone) {
    return safeRedis(false, async () => Boolean(await redisClient.get(kitchenCheckoutGraceKey(instanceId, phone))));
}
export async function getKitchenCheckoutFingerprint(instanceId, phone) {
    return safeRedis(null, async () => {
        const value = await redisClient.get(kitchenCheckoutGraceKey(instanceId, phone));
        return value ? String(value) : null;
    });
}
export async function clearKitchenCheckoutState(instanceId, phone) {
    await safeRedis(undefined, async () => {
...
function historyRole(entry) {
    ...
    if (role === "operator" || source === "operator_panel" || source === "whatsapp_app")
        return "operator";
    if (["assistant", "model", "bot", "ai"].includes(role) || entry?.direction === "outgoing" || entry?.fromMe === true)
        return "assistant";
    if (role === "system")
        return "system";
    return "user";
...
function mergeConversationHistory(openbotRows, whatsProRows) {
    ...
    for (const entry of combined) {
        ...
        if (duplicateIndex >= 0) {
            if (role === "operator" && historyRole(result[duplicateIndex]) !== "operator")
                result[duplicateIndex] = entry;
            if (id)
                ids.add(id);
            continue;
        }
...
export function siteLanguageHintKey(instanceId, phone) {
...
export async function getUserLangState(instanceId, phone) {
    return safeRedis({ language: null, ttlSeconds: -2, siteHint: null }, async () => {
...
export async function clearUserLang(instanceId, phone) {
...
export async function getSiteLanguageHint(instanceId, phone) {
    return safeRedis(null, async () => {
        const value = await redisClient.get(siteLanguageHintKey(instanceId, phone));
        return value === "kk" || value === "ru" ? value : null;
    });
}
export async function saveSiteLanguageHint(instanceId, phone, lang) {
    return safeRedis(false, async () => {
...
function complaintClarifyKey(instanceId, phone) {
    return `complaint_clarify:${instanceId}:${phone}`;
}
export async function markComplaintClarificationPending(instanceId, phone, text) {
    return safeRedis(false, async () => {
...
export async function takeComplaintClarification(instanceId, phone) {
    return safeRedis(null, async () => {
...
export async function hasComplaintClarificationPending(instanceId, phone) {
...
export async function deleteShiftNote(instanceId, noteId, text = "") {
    await safeRedis(undefined, async () => {
        ...
        if (safeNoteId && safeNoteId !== "0")
            await deleteKey(`shift_note:${instanceId}:${safeNoteId}`);
        if (!deletedIds.length && expectedText) {
            ...
            for (const key of keys) {
                ...
                if (stored.text.toLowerCase().trim() === expectedText)
                    await deleteKey(key);
            }
        }
