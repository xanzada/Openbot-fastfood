...
function extractPhoneCandidate(rawValue) {
  ...
  const phoneLikeMatch = raw.match(/(?:\+?7|8)[\s().-]*\d{3}[\s().-]*\d{3}[\s().-]*\d{2}[\s().-]*\d{2}/) || raw.match(/\d{10,15}/);
...
async function normalizePublicDomain(rawDomain = "") {
  ...
  const records = await Promise.race([
    ...
    new Promise((_, reject) => setTimeout(() => reject(new Error("DNS_TIMEOUT")), 5e3))
  ]);
...
async function apiBot(domain, payload, timeout = 1e4) {
...
function normalizeRuntimeStatus(data = {}) {
  ...
  const nested = safeJsonObject(data.runtime_status, null) || safeJsonObject(data.kitchen_status, null) || safeJsonObject(data.status, null) || data;
...
function runtimeFromKitchenStatus(instanceId, status) {
  ...
  const closedReason = status.is_emergency ? "emergency_stop" : !status.delivery && !status.pickup ? "service_channels_disabled" : "";
  return {
...
async function getRuntimeStatus(instanceId, domain, options = {}) {
  ...
  try {
    const data = await apiBot(domain, { action: "get_runtime_status", restaurant_id: instanceId }, 8e3);
    ...
    if (redisKitchen) {
      ...
      await setJsonCache(cacheKey, 5, redisRuntime).catch(() => void 0);
      return redisRuntime;
    }
...
function normalizeOrderPayload(order = {}) {
  ...
  return {
    ...
    ai_comment: String(order.ai_comment || "").trim().slice(0, 1e3),
...
const inactiveStatuses = /* @__PURE__ */ new Set(["completed", "done", "finished", "closed", "cancelled", "canceled", "refunded"]);
function isInactiveOrderStatus(status = "") {
  return inactiveStatuses.has(String(status || "").trim().toLowerCase().replace(/\s+/g, "_"));
}
async function getOrderStatus(instanceId, phone, domain) {
...
async function getOrderContext(instanceId, domain, options = {}) {
  ...
  const key = orderId > 0 ? `order_context:${instanceId}:id:${orderId}` : `order_context:${instanceId}:phone:${cleanPhone}`;
  try {
    const data = await apiBot(
      domain,
      {
...
async function getMenuContext(instanceId, domain, userLang = "kk") {
  ...
  if (cached) return cached;
  try {
    const data = await apiBot(domain, { action: "get_menu_context", restaurant_id: instanceId, lang }, 1e4);
...
function normalizeReceiptSenderName(value = "") {
  ...
  if (!text || /[^\p{L}\s.'’\-]/u.test(text) || nameParts.length < 2 || /^(payment\s*link|pay\s*link|qr|kaspi|halyk|unknown|sender|жіберуші|жіберуші аты|белгісіз|отправитель)$/iu.test(
    normalized
  )) {
    return "\u0411\u0435\u043B\u0433\u0456\u0441\u0456\u0437";
  }
...
function buildReceiptCrmPayload(data) {
...
async function sendOperatorSosSignal(input) {
...
async function updateCrmAction(actionType, instanceId, phone, data) {
  ...
  try {
    const response = await apiBot(domain, payload, 1e4);
