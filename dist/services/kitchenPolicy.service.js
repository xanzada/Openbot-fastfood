...
export function formatKitchenWait(minutesValue, language) {
    ...
    if (!rest)
        return language === "kk" ? `${hours} сағат` : `${hours} ${hours === 1 ? "час" : hours < 5 ? "часа" : "часов"}`;
    return language === "kk"
        ? `${hours} сағат ${rest} минут`
        : `${hours} ${hours === 1 ? "час" : hours < 5 ? "часа" : "часов"} ${rest} минут`;
}
export function classifyKitchenSalesPolicy(runtime, nowMs = Date.now()) {
    ...
    let mode = "normal";
    if (globalStop && resetAt === 0)
        mode = "indefinite";
    else if (globalStop && remainingSeconds >= 86400)
        mode = "vacation";
    else if (globalStop || waitMinutes > 180)
        mode = "critical";
    else if (waitMinutes > 40)
        mode = "busy";
    else if (delivery !== pickup)
        mode = "channel_limited";
...
export function detectKitchenConsentAnswer(text = "") {
    const value = String(text || "").toLowerCase().replace(/[.,!?;:]+/g, " ").replace(/\s+/g, " ").trim();
    if (/(^|\s)(жоқ|жок|керек емес|күтпеймін|кутпеймин|нет|не надо|не буду ждать|отмена|рахмет жоқ|спасибо нет)(\s|$)/iu.test(value))
        return "no";
    if (/(^|\s)(иә|ия|иа|жарайды|келісемін|келисемин|күте аламын|куте аламын|күтемін|кутемин|да|хорошо|согласен|согласна|подожду|буду ждать)(\s|$)/iu.test(value))
        return "yes";
    return "unknown";
}
export function detectRequestedServiceChannel(text = "") {
    const value = String(text || "");
    if (/(самовывоз|алып\s*кет|өзім\s*алам|озим\s*алам|pickup)/iu.test(value))
        return "pickup";
    if (/(достав|жеткіз|жеткиз|курьер|delivery)/iu.test(value))
        return "delivery";
    return "unknown";
}
