...
function reportGlobalFailure(scope, error, meta = {}) {
  ...
  void notifyAllDevelopersSystemFailure(error, { scope, ...meta }).catch(() => void 0);
}
...
app.use((error, req, res, _next) => {
  ...
  if (instanceId) {
    void notifyDeveloperSystemFailure(instanceId, error, meta).catch(() => void 0);
  } else {
    void notifyAllDevelopersSystemFailure(error, meta).catch(() => void 0);
  }
...
httpServer.listen(port, () => {
  ...
  void logStartupDiagnostics().then((checks) => Promise.all(
    checks.filter((check) => !check.ok).map((check) => notifyAllDevelopersSystemFailure(new Error(check.message || `${check.name} unavailable`), {
