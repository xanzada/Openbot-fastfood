...
export function mergeStreamResponse(response, chunk) {
    ...
    data.intermediate_failures = mergeIntermediateFailures(data.intermediate_failures, chunk.intermediate_failures);
    data.final_result = mergeLlmModule(data.final_result, chunk.final_result);
}
function mergeIntermediateFailures(existing, incoming) {
    if (!incoming || incoming.length === 0) {
        return existing;
    }
    const transformedIncoming = incoming.map(error => ({
        ...error,
    ...
    const existingSet = new Set(existing.map(f => JSON.stringify([f.request_id, f.code, f.message])));
    const newFailures = transformedIncoming.filter(f => !existingSet.has(JSON.stringify([f.request_id, f.code, f.message])));
