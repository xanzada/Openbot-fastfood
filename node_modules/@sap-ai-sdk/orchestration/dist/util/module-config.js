...
function warnAboutUnusedOverrides(streamOptions, configurationCount) {
    ...
    if (unusedOverrides.length) {
        logger.debug(`The following override keys do not correspond to any module configuration and will be ignored: ${unusedOverrides.join(', ')}.`);
    }
}
function warnAboutShortOverridesArray(streamOptions, configurationCount) {
    if (!Array.isArray(streamOptions?.overrides) ||
        !streamOptions.overrides.length) {
        return;
    }
    const arrayLength = streamOptions.overrides.length;
    logger.debug(`Override array has ${arrayLength} element(s) but there are ${configurationCount} module configuration(s). ` +
 ...
 */
function getStreamOptionsForItem(streamOptions, index) {
    if (!streamOptions) {
        return undefined;
    }
 ...
 */
function warnAboutMissingOutputFiltering(moduleConfigs, streamOptions) {
    ...
    const configsWithoutFilter = configs
        .map((cfg, idx) => {
    ...
    if (configsWithoutFilter.length === configs.length) {
        logger.warn('Output filter stream options are not applied because no module configuration has output filtering enabled.');
    }
    else if (configsWithoutFilter.length > 0) {
        const configWord = configsWithoutFilter.length > 1 ? 'configurations' : 'configuration';
        const positions = configsWithoutFilter.map(i => `#${i + 1}`).join(', ');
        logger.warn(`Output filter stream options will not be applied to ${configWord} ${positions} because output filtering is not configured for those modules.`);
    }
}
/**
 * @internal
 ...
 */
function buildModules(configs, options) {
...
export function addStreamOptions(moduleConfigs, streamOptions) {
    ...
    if (!Array.isArray(moduleConfigs) && streamOptions?.overrides) {
        throw new Error('Overrides in stream options are not supported when a single module configuration is provided.');
    }
...
function addStreamOptionsToSingleModuleConfig(moduleConfigs, streamOptions) {
...
export function constructCompletionPostRequest(config, request, stream, streamOptions) {
    ...
    const routeMessagesToHistory = configs.some(c => isTemplateRef(c?.promptTemplating?.prompt || {}));
    const moduleRequest = routeMessagesToHistory && request
    ...
    const messagesHistory = routeMessagesToHistory && request?.messages?.length
        ? [...(request.messagesHistory || []), ...request.messages]
        : request?.messagesHistory;
    return {
...
function isTemplateRef(templating) {
    return (!!templating &&
        typeof templating === 'object' &&
        'template_ref' in templating);
}
