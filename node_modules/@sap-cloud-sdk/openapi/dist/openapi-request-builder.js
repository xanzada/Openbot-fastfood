...
class FormDataBuilder {
    ...
    build() {
        ...
        for (const [key, value] of Object.entries(this.body ?? {})) {
            if (!this?.encoding[key]) {
                throw new Error(`Missing encoding metadata for property '${key}'. ` +
                    'This indicates a code generation issue. ' +
                    'Please regenerate your API client.');
            }
            const metadata = this.encoding[key];
            const allowedTypes = new Set(metadata.parsedContentTypes.map(ct => ct.type.toLowerCase()));
            const encoded = value instanceof Blob
    ...
    checkIsFlexibleContentType(metadata, allowedTypes) {
     ...
     */
    encodeBlob({ key, value }, metadata) {
        const { contentType: targetContentType, isImplicit: targetIsImplicit, parsedContentTypes } = metadata;
        const allowedTypes = new Set(parsedContentTypes.map(ct => ct.type.toLowerCase()));
        ...
        if (!value.type &&
            ...
            logger.debug(`Adding missing content type '${targetContentType}' to Blob for key '${key}' as per encoding specification.`);
        ...
        !targetIsImplicit &&
            ...
            logger.warn(`Content type mismatch for key '${key}': value has type '${value.type}' but encoding specifies '${targetContentType}'.`);
        }
        return value;
    }
    /**
     ...
     */
    encodeString({ key, value }, metadata, allowedTypes) {
        // Handle string data
        ...
        const targetCharset = (0, util_1.unique)(metadata.parsedContentTypes.map(ct => ct.parameters.charset));
        ...
        let buffer;
        try {
            buffer = Buffer.from(stringValue, targetCharsetValue);
        }
        catch (e) {
            throw new util_1.ErrorWithCause(`Failed to encode form data field '${key}' with charset '${targetCharsetValue}'.`, e);
        }
...
class OpenApiRequestBuilder {
    ...
    getBody() {
        ...
        const contentType = (0, util_1.pickValueIgnoreCase)(this.parameters?.headerParameters, 'content-type');
