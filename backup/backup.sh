...
next_slot() {
  ...
  if [[ "${current}" == 'a' ]]; then printf 'b'; else printf 'a'; fi
}
...
publish_snapshot() {
  ...
  # Each run orphans the previous snapshot commit, but its blob stays reachable
  # through the HEAD reflog, so plain gc never collects it and the repo grows by
  # a full snapshot every cycle. Drop the reflog first, then gc.
